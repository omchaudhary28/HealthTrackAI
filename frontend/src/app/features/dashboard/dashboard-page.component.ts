    ...
    <ng-container *ngIf="summary$ | async as summary; else loading">
      <ng-container *ngIf="summary.error; else content">
        <div class="rounded-3xl border border-red-200 bg-red-50 p-6 text-red-700">
          <div class="comic-empty-state border-red-200/70 bg-red-50/80">
            <div class="comic-empty-illustration"></div>
            <div>
              <p class="font-semibold">Could not load dashboard summary.</p>
              <p class="text-sm">We will try again on refresh.</p>
            </div>
          </div>
        </div>
      </ng-container>
        ...
        <section appScrollReveal class="page-stack">
          <div class="mt-card mt-card-hover page-hero">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
              <div class="mt-card-brand min-w-0 max-w-3xl">
                <div class="mt-card-icon">
                  <app-icon name="dashboard" className="text-xl icon-bounce-soft"></app-icon>
                </div>
                <div class="min-w-0">
            ...
            <div class="mt-8 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
              ...
              <div class="mt-card-strong mt-card-hover comic-corner-doodle p-6">
                <div class="mt-card-brand">
                  <div class="mt-card-icon">
                    <app-icon name="target" className="text-lg icon-bounce-soft"></app-icon>
                  </div>
                  <div>
                ...
                <div class="comic-suggestion-note">
                  {{ friendlyActionLine(summary) }}
                </div>
          ...
          <div class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
            ...
            <div class="space-y-6">
              ...
              <div class="theme-bento-card-soft rounded-[1.75rem] p-5 sm:rounded-[2rem] sm:p-6">
                <div class="mt-card-brand">
                  <div class="mt-card-icon h-11 w-11 rounded-[0.95rem]">
                    <app-icon name="sparkles" className="text-base icon-bounce-soft"></app-icon>
                  </div>
                  <div class="text-sm font-semibold text-slate-900">AI notes</div>
                </div>
                <div class="mt-4 space-y-3">
                  <ng-container *ngIf="(summary.aiInsightsHistory || []).length; else emptyInsights">
                ...
                <ng-template #emptyInsights>
                  <div class="comic-empty-state mt-4">
                    <div class="comic-empty-illustration"></div>
                    <div class="text-sm text-slate-600">No fresh insights yet. Log one check-in and we will surface a quick read.</div>
                  </div>
                </ng-template>
              </div>
            </div>
          ...
          <div class="glass-card theme-bento-card-soft rounded-[2rem] p-5 sm:rounded-[2.25rem] sm:p-6">
            ...
            <div class="mt-5 grid gap-4 lg:grid-cols-2">
              <ng-container *ngIf="(summary.recommendationCards || []).length; else noRecommended">
                <article *ngFor="let ex of (summary.recommendationCards || []).slice(0, 4)" class="theme-bento-card comic-corner-doodle rounded-[2rem] p-5">
                  <div class="flex items-start justify-between gap-4">
                    <div class="mt-card-brand min-w-0">
                      <div class="mt-card-icon h-11 w-11 rounded-[0.95rem]">
                        <app-icon [name]="ex.category === 'breathing' ? 'spa' : ex.category === 'stress-release' ? 'activity' : 'heartbeat'" className="text-base icon-bounce-soft"></app-icon>
                      </div>
                      <div class="min-w-0">
                  ...
                  <div class="mt-2 text-xs text-slate-500">Quick note: start easy and check how your body responds.</div>
              ...
              <ng-template #noRecommended>
                <div class="comic-empty-state lg:col-span-2">
                  <div class="comic-empty-illustration"></div>
                  <div class="text-sm text-slate-600">No recommendations yet. Complete one check-in and we will suggest a fit.</div>
                </div>
              </ng-template>
            </div>
          </div>
...
export class DashboardPageComponent {
  ...
  friendlyActionLine(summary: DashboardSummary): string {
    const stress = lastValue(summary.analytics?.stressTrend?.values, 55);
    if (stress >= 60) {
      return "Let's keep this one gentle today. Small step, clear head.";
    }

    return "You're doing fine. Keep the momentum with one focused reset.";
  }
